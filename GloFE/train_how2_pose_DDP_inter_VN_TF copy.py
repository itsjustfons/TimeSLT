...
from utils.model_io import ( #from WenChin's seq2seq code
    filter_modules,
...
class OpenASLPoseDataset(Dataset):
    def __init__(self, arg_dict, tokenizer, phase='train', split='train'):
        ...
        def filter_missing(row):
            if arg_dict.get('vid_format') == "mp4":
                full_path = os.path.join(self.feat_path, f'{row["SENTENCE_NAME"]}.mp4')
            elif arg_dict.get('vid_format') == "jpg":
                full_path = os.path.join(self.feat_path, f'{row["SENTENCE_NAME"]}')
        ...
        self.translation = df_filtered['SENTENCE'].to_list()
        self.video_names = df_filtered['SENTENCE_NAME'].to_list()
        
        self.vn_vocab = 5523
        self.matched_VNs = json.load(open('/data/group1/z40575r/TimeSformer_GloFE/GloFE/notebooks/how2sign/uncased_filtred_glove_VN_matched_train.json', 'r'))
        self.vn_to_idx = {}
        with open('/data/group1/z40575r/TimeSformer_GloFE/GloFE/notebooks/how2sign/uncased_filtred_glove_VN_idxs.txt', 'r') as f:
            content = f.readlines()
            for line in content:
        ...
        for trans in self.translation:
            ...
            self.labels.append(trans_ids) #not making it a tensor
        ...
        self.video_format = arg_dict.get('vid_format')
    ...
    def read_video(self, index):
        ...
        if self.video_format == "mp4": 
            video_path = os.path.join("/data/group1/z40575r/TimeSformer_GloFE/TimeSformer/datasets/how2sign/256/" + fi + ".mp4")
        ...
        elif self.video_format == "jpg":
            img_folder = os.path.join("/data/group1/z40575r/CorrNet/gradcam/" + fi + "/*.jpg")
            #print("fi:", fi)
            image_list = sorted(glob.glob(img_folder))
            img_list=[]
            for frame_path in image_list:
                img = cv2.imread(frame_path)
                if img is None:
                    print(f"faild to load {frame_path}")
                    continue
    ...
    def transform(self):
        ...
        if self.phase == 'train':
            ...
            return video_augmentation.Compose([
                ...
                video_augmentation.ToTensor(), #This converts the video to a tensor
    ...
    #normalize function from CorrNet
    def normalize(self, video, label, file_id=None):
        if label is not None:
            video, label = self.data_aug(video, label, file_id)
            video = video.float() / 127.5 - 1
            #print("normalize video shape", video.shape)
            return video, label
    ...
    def __getitem__(self, index: int):
        if self.phase == 'train':
            ...
            text_tokens, mask, token_length = self.pad_token_ids(
                index)  # [max_seq_len]
            #try:
            visual_prefix, visual_length, labels = self.read_video(index) #read_video from CorrNet

            if not visual_prefix or visual_length == 0:
                print(f"Skipping empty video at index {index}")
                return None
            visual_prefix, label = self.normalize(visual_prefix, labels)
            print("after normalize", visual_prefix.shape)
            video = (visual_prefix,) #Use this to pack it into a batch. Need to fix when doing multiple videos within a batch
            left_pad = 0
            last_stride = 1
            total_stride = 1
            global kernel_sizes 
            kernel_sizes = ['K5', "P2", 'K5', "P2"]
            for layer_idx, ks in enumerate(kernel_sizes): #layer_idx does not seem to be used. Same in the original code
                if ks[0] == 'K':
            ...
            if len(video[0].shape) > 3:
                ...
                #print("printing video_length: ", video_length)
            ...
            visual_prefix = padded_video
            vn_idxs, vn_len = self.get_vn(index)

            return text_tokens, mask, visual_prefix, token_length, visual_length, vn_idxs, vn_len

            #except Exception as e:
                #Handle unexpected erros during video reading or preprocessing
            #    print(f"Error at index {index}: {e}")
            #    print("Error filename:", self.video_names[index])
            #    return None
        
        elif self.phase == 'test':
            #visual_prefix, visual_length = self.read_pose_files(index)
            try:    
                visual_prefix, visual_length, labels = self.read_video(index)
                text_tokens, mask, token_length = self.pad_token_ids(index)  # [max_seq_len]

                visual_prefix = self.normalize(visual_prefix, labels)
                visual_prefix = visual_prefix[0] #extract tensor from tuple
                #print("visual prefix after normalization", visual_prefix.shape)
                video = (visual_prefix,) #Use this to pack it into a batch. Need to fix when doing multiple videos within a batch
                #video = ("visual_prefix shape test", visual_prefix.shape)
                visual_length = torch.LongTensor([len(vid) for vid in video])
                #Turn off GloFE data augmentation. We will use CorrNet's data augmentation
                '''
                ...
                '''
                visual_prefix = torch.stack([visual_prefix], dim = 0)
                #print("visual_prefix shape end of getitem", visual_prefix.shape)

                #convert text_tokens and token_length into a tensor
                #text_tokens = torch.tensor(text_tokens)
                #token_length = torch.tensor(token_length)

                return visual_prefix, index, visual_length, text_tokens, token_length
            except Exception as e:
                #Handle unexpected erros during video reading or preprocessing
                print(f"Error at index {index}: {e}")
                #print("Error filename:", self.video_names[index])
...
def train(datasets, model, args,
          lr: float = 2e-5, warmup_steps: int = 5000, output_dir: str = ".", output_prefix: str = "", accumulation_steps: int = 24):#accum steps = 24 *12
    ...
    for epoch in range(epoch_st, epochs):
        ...
        if (epoch > -1):
            ...
            #turn off test dataset when trianing for now
            ...
            print("first save function triggered")
    ...
    print("last save function triggered")
    return model
...
def eval(dataset, model, args, device, split, save_results: bool = False,
    ...
    eval_dataloader = DataLoader(
        dataset, batch_size=1, shuffle=False, drop_last=False, num_workers=8) #originally batch_size = args.bs
    ...
    with torch.no_grad():
        for idx, (visual_prefix, text_label_index, visual_length, tokens, token_length) in enumerate(eval_dataloader):
...
def main():
    ...
    #pre-loade decoder weights
    parser.add_argument('--dec_pre_trained' ,default = "False", help='load pre-trained glofe decoder weights')
    
    # Overlap for TimeSformer
    parser.add_argument('--ts_overlap', type=int, default=2, help='overlapping frames for the timesformer sliding window')

    # Vid format
    parser.add_argument('--vid_format', type=str, default='mp4', help='video format of input data. either mp4 or jpg')
    ...
    #self.video_format = args.vid_format
    ...
    if args.phase == 'train':
        ...
        #retrieve the corrnet weights
        print("dec pre trained", args.dec_pre_trained)
        if args.dec_pre_trained == "True":
            ...
            print("is it initializing the decoder incorrectly for training resume?")

        #checking for trainable and frozen layers before training
        frozen_layers = []
        trainable_layers = []
